...
test "parseOrFail names the format and the line" {
    ...
    try testing.expect(std.mem.startsWith(u8, fail.text(), "skl.yaml is not valid YAML: "));
...
test "needsQuoting quotes only what would read back as something else" {
    ...
    try testing.expect(!yaml.needsQuoting("skl.yaml"));
    try testing.expect(!yaml.needsQuoting("/tmp/x/.config/skilled/skl.yaml"));
