...
// misunderstands is the wrong packages cloned and linked.
//
...
const Parser = struct {
    ...
    fn parseScalar(self: *Parser, text: []const u8, line: usize, column: usize) Error!Value {
        ...
        switch (trimmed[0]) {
            ...
            '&', '*', '!', '|', '>', '%', '@', '`' => {
                return self.syntax(line, column, "this YAML feature is not supported");
            },
    ...
    while (walker.next()) |raw_line| {
        ...
        if (std.mem.eql(u8, content, "---")) {
            if (lines.items.len > 0) {
                err.* = .{ .message = "more than one document in the file, which this parser does not support", .line = number, .column = indent + 1 };
                return error.Syntax;
            }
            continue;
        }
