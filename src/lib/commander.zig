...
// against.
//
// It is an implementation of what is used, not of commander. Boolean flags, short flags, and merging
// parent option values into a subcommand are included because `skl -g add` and `skl add -g` both
// have to work. Missing on purpose: option value parsers, `.requiredOption`, `.hook`, clustered
// shorts (`-gn`), variadic options, and negatable `--no-` forms (`--no-color` is its own flag).
//
...
//
// One option an action can read: a `--name <value>` or a boolean flag with no placeholder.
//
pub const Option = struct {
    ...
    //
    // The name an action looks it up by: the long flag with the leading dashes and the placeholder
    // taken off, so "--config <path>" is read as "config" and "-g, --global" is read as "global".
    //
    pub fn name(self: Option) []const u8 {
        var fallback: []const u8 = self.flags;
        var spellings = std.mem.splitSequence(u8, self.flags, ", ");
        while (spellings.next()) |spelling| {
            var spelled = spelling;
            if (std.mem.indexOfScalar(u8, spelled, ' ')) |space| {
                spelled = spelled[0..space];
            }
            if (std.mem.startsWith(u8, spelled, "--")) {
                return std.mem.trimStart(u8, spelled, "-");
            }
            fallback = spelled;
        }
        return std.mem.trimStart(u8, fallback, "-");
    }

    //
    // True when the flags string names a value, such as "--ns <namespace>".
    //
    // A flags string with no `<placeholder>` is a boolean: presence stores "true", absence is null
    // unless a default was set. Public so the tests in commander.test.zig can reach it.
    //
    pub fn takesValue(self: Option) bool {
        return std.mem.indexOfScalar(u8, self.flags, '<') != null;
    }
};
...
pub const Command = struct {
    ...
    //
    // The command this was added to, so an unknown option can be looked up on the parent chain and
    // values collected on the parent can be merged into the child.
    //
    parent: ?*Command = null,
    ...
    //
    // The option a flag names on this command or an ancestor, or null.
    //
    // Walking the parent chain is what lets `--global` be declared once on the root and still be
    // accepted as `skl add -g`.
    //
    pub fn findOption(self: *const Command, flag: []const u8) ?Option {
        var at: ?*const Command = self;
        while (at) |current| {
            if (findOwnOption(current, flag)) |found| return found;
            at = current.parent;
        }
        return null;
    }
...
//
// The option a flag names on this command alone, ignoring ancestors.
//
fn findOwnOption(command: *const Command, flag: []const u8) ?Option {
...
//
// The version string on this command or an ancestor, if `flag` is a version spelling there.
//
fn versionTextForFlag(command: *const Command, flag: []const u8) ?[]const u8 {
    var current: ?*const Command = command;
    while (current) |c| {
        if (c.isVersionFlag(flag)) return c.version_text;
        current = c.parent;
    }
    return null;
}
...
//
// The version string on this command or an ancestor, used by the `version` command name.
//
fn versionTextOnChain(command: *const Command) ?[]const u8 {
    var current: ?*const Command = command;
    while (current) |c| {
        if (c.version_text) |text| return text;
        current = c.parent;
    }
    return null;
}
...
pub fn parse(runner: *Program, root: *Command, argv: []const []const u8) Error!void {
    ...
    while (at < argv.len) : (at += 1) {
        ...
        if (!options_ended and isOption(word)) {
            ...
            if (versionTextForFlag(current, word)) |text| {
                runner.out.print("{s}\n", .{text}) catch {};
                return error.Displayed;
            }
            ...
            if (!found.takesValue()) {
                try values.put(allocator, found.name(), "true");
                continue;
            }
        ...
        if (positionals.items.len == 0) {
            if (current.findSubcommand(word)) |sub| {
                //
                // Descending into a subcommand. Values gathered so far are kept, which is how
                // `skl -g add` still has `--global` set when the add action runs.
                //
                current = sub;
                positionals = .empty;
                continue;
            ...
            if (current.subcommands.items.len > 0 and std.mem.eql(u8, word, "help")) {
            ...
            if (current.subcommands.items.len > 0 and std.mem.eql(u8, word, "version")) {
                if (versionTextOnChain(current)) |text| {
                    runner.out.print("{s}\n", .{text}) catch {};
                    return error.Displayed;
                }
            }
    ...
    //
    // Any option with a default that was not given takes that default, walking from the chosen
    // command up to the root so a root default still reaches a subcommand action.
    //
    var defaults_at: ?*const Command = current;
    while (defaults_at) |command| {
        for (command.options.items) |candidate| {
