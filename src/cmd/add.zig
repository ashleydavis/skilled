//
// `skl add`: clone, scan, append YAML, then link — or append YAML from `--from`.
//
// Clone and scan run before any YAML write. `--from` fetches a YAML file in git and merges it;
// it does not clone skill packages or create links.
//
...
pub fn run(ctx: *const Context, args: Args) skilled.failure.Error!u8 {
    ...
    const resolved = try shared.ensureCloned(ctx, spec, &spinner);
    _ = try package.scan(ctx.io, ctx.allocator, resolved.dest, ctx.fail);

    const appended = try appendPackage(ctx.allocator, file, spec, namespace);
...
fn appendPackage(
    ...
    for (file.packages, 0..) |pkg, i| {
        packages[i] = pkg;
    }
    packages[file.packages.len] = .{ .repo = repo, .namespace = namespace };
